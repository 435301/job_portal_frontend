...
import { createCompanyType } from "../../redux/slices/CompanyTypeSlice.tsx";
import { FormErrors, validateCompanyTypeForm } from "../../common/validation.tsx";
...
function CreateCompanyType() {
    ...
    const [formData, setFormData] = useState<CompanyTypeForm>({
        companyType: "",
        status: "",
        ipAddress: "",
    });
    ...
    const handleSubmit = async (e: FormEvent) => {
        e.preventDefault();
        const validationErrors = validateCompanyTypeForm(formData);
        ...
        try {
            const resultAction = await dispatch(createCompanyType(finalData));
            if (createCompanyType.fulfilled.match(resultAction)) {
                navigate("/admin/manage-company-type");
            }
        } catch (error) {
            console.log(error)
        }
                            ...
                            <Row className="align-items-center">
                                ...
                                <Col xs="auto">
                                    <Button
                                        ...
                                        onClick={() => navigate("/admin/manage-company-type")}
                                    >
                                        <i className="bi bi-arrow-left me-2"></i>Manage Company Type
                                    </Button>
                                </Col>
                            </Row>
                        </Card>
