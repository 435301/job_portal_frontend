...
import { deleteRole, getAllRole, updateRole } from "../../redux/slices/RoleSlice.tsx";
import RoleViewModal from "../componets/Modal/RoleViewModal.tsx";
import RoleEditModal from "../componets/Modal/RoleEditModal.tsx";
...
function RoleManage() {
    ...
    const { RoleList, loading } = useSelector(
    ...
    const [RoleDelete, setRoleDelete] = useState<number | null>(null);
    ...
    useEffect(() => {
        dispatch(getAllRole({ page, search: searchTerm, status: statusFilter ,}));
    ...
    const handleReset = () => {
        ...
        setStatusFilter("");
        setPage(1);
        dispatch(getAllRole({ page: 1, search: "", status: "" ,}));
    };
    ...
    const handleSaveEdit = (updatedItem: Role) => {
        dispatch(updateRole({ id: updatedItem.id, updateData: updatedItem }));
        setShowEdit(false);
    };
    ...
    const handleDelete = async () => {
        if (RoleDelete !== null) {
            await dispatch(deleteRole(RoleDelete));
                        ...
                        <Card className="p-3 border-0 card-header shadow-sm mb-2">
                            <Row className="align-items-center">
                                ...
                                <Col xs="auto">
                                    ...
                                    >
                                        <i className="bi bi-plus-circle me-2"></i>Add Role
                                    </Button>
                                </Col>
                            </Row>
                        </Card>
                        ...
                        <Card className="shadow-sm border-0">
                            <Card.Header className="bg-white py-3 d-flex justify-content-between align-items-center">
                                <h6 className="fw-bold mb-0 text-primary">
                                    <i className="bi bi-list-task me-2"></i>Role List
                                </h6>
                                <span className="text-muted small">
                                    Showing {RoleList?.length || 0} results
                                </span>
                            </Card.Header>
                            ...
                            <Card.Body className="p-0">
                                {loading ? (
                                    ...
                                    <Table responsive hover className="mb-0 align-middle">
                                        ...
                                        <tbody>
                                            {RoleList?.length > 0 ? (
                                                RoleList.map((item: Role, index: number) => (
                                                    <tr key={item.id}>
                                                        <td>{index + 1}</td>
                                                        <td>{item.role}</td>
                                                        <td>
