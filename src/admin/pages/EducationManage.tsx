...
import EducationViewModal from "../componets/Modal/EducationViewModal.tsx";
import EducationEditModal from "../componets/Modal/EducationEditModal.tsx";
import "bootstrap-icons/font/bootstrap-icons.css";
import { useDispatch, useSelector } from "react-redux";
import { deleteEducation, getAllEducations, updateEducation } from "../../redux/slices/educationSlice.tsx";
...
function EducationManage() {
    ...
    const { educationList, loading } = useSelector(
    ...
    const [educationDelete, setEducationDelete] = useState<number | null>(null);
    ...
    useEffect(() => {
        dispatch(getAllEducations({ page, search: searchTerm, showStatus: statusFilter }));
    ...
    const handleReset = () => {
        ...
        setStatusFilter("");
        setPage(1);
        dispatch(getAllEducations({ page: 1 }));
    };
    ...
    const handleSaveEdit = (updatedItem: Education) => {
        dispatch(updateEducation({ id: updatedItem.id, updateData: updatedItem }));
        setShowEdit(false);
    };

    const handleDeleteClick = (id: any) => {
        setEducationDelete(id);
        setShowDeleteModal(true);
    };

    const handleDelete = async () => {
        if (educationDelete !== null) {
            await dispatch(deleteEducation(educationDelete));
            setShowDeleteModal(false);
            setEducationDelete(null);
        }
    };
                            ...
                            <Row className="align-items-center g-3">
                                <Col md={3}>
                                    <InputGroup>
                                        <Form.Control
                                            type="text"
                                            placeholder="Search education name..."
                                            value={searchTerm}
                        ...
                        <Card className="shadow-sm border-0">
                            <Card.Header className="bg-white py-3 d-flex justify-content-between align-items-center">
                                <h6 className="fw-bold mb-0 text-primary">
                                    <i className="bi bi-list-task me-2"></i>Education List
                                </h6>
                                <span className="text-muted small">
                                    Showing {educationList?.length || 0} results
                                </span>
                            </Card.Header>
                                    ...
                                    <Table responsive hover className="mb-0 align-middle">
                                        ...
                                        <tbody>
                                            {educationList?.length > 0 ? (
                                                educationList.map((item: Education, index: number) => (
                                                    <tr key={item.id}>
                                                        <td>{index + 1}</td>
